...
const Scanner = () => {
    ...
    const fetchProductDetails = async (barcode) => {
        setLoading(true);
        try {
            const response = await axios.get(`http://10.125.19.134:5000/scan?barcode=${barcode}`);
    ...
    const checkBarcodeInDatabase = async (barcode) => {
      ...
      setLoading(true);
      try {
          const response = await axios.get(`http://10.125.19.134:5000/check_barcode?barcode=${barcode}`);
